class User::DashboardController < User::BaseController
  ...
  def fire_donate_request
		if Match.where(user_id: current_user.id, completion_status: 'pending').length > 1
			...
			prospective_up_links = User.where(status: :activated).sort_by(&:updated_at).select { |user| Match.where(matched_user_id: user.id).length < 2 && (UserPackage.find_by(user_id: user.id).package_id == UserPackage.find_by(user_id: current_user.id).package_id) }
